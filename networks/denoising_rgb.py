...
def conv(in_channels, out_channels, kernel_size, bias=True, padding = 1, stride = 1):
    return nn.Conv2d(in_channels, out_channels, kernel_size, padding=(kernel_size//2), padding_mode='reflect', bias=bias, stride=stride)
...
class RRG(nn.Module):
    def __init__(self, conv, n_feat, kernel_size, act,  num_dab):
        super(RRG, self).__init__()
        modules_body = []
        modules_body = [DAB(conv, n_feat, kernel_size, bias=True, bn=False, act=act) for _ in range(num_dab)]
...
class DenoiseNeXt(nn.Module):
    def __init__(self, conv=conv):
        super(DenoiseNeXt, self).__init__()
        n_feats = 96
        kernel_size = 3
        inp_chans = 3 
        act = nn.GELU()
        ...
        modules_body = [RRG(conv, n_feats, kernel_size, act=act, num_dab=3),
                        RRG(conv, n_feats, kernel_size, act=act, num_dab=3),
                        RRG(conv, n_feats, kernel_size, act=act, num_dab=9),
                        RRG(conv, n_feats, kernel_size, act=act, num_dab=3)]
