...
parser.add_argument('--input_dir', default='./datasets/asus_train/asus_train_rgb/clean/', type=str, help='Directory of clean RGB images')
parser.add_argument('--result_dir', default='./results/synthesized_cropped/', type=str, help='Directory for results')
...
with torch.no_grad():
    for ii, data in enumerate(tqdm(test_loader), 0):
        ...
        for j in range(raw_gt.shape[0]):   ## Use loop to add different noise to different images.
            ...
            #rgb_clean = model_raw2rgb(raw_gt[j].unsqueeze(0), ccm_tensor) 
            #rgb_clean = torch.clamp(rgb_clean, 0, 1)

            rgb_noisy = rgb_noisy.permute(0, 2, 3, 1).squeeze().cpu().detach().numpy()
            #rgb_clean = rgb_clean.permute(0, 2, 3, 1).squeeze().cpu().detach().numpy()
